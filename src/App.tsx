...
import {
  generateBrief,
  type BriefDetailMode,
  type BriefDuration,
} from "./logic/generateBrief";
import {
  createDecisionEvent,
  createHumanDecision,
  createSystemEvent,
} from "./logic/governance";
...
function createTaxonomyChange(
  change: Omit<TaxonomyChange, "id" | "timestamp">,
): TaxonomyChange {
  return {
...
const panelClass = "rounded-md border border-line bg-surface shadow-panel";
const panelSoftClass = "rounded-md border border-line bg-surface-soft";
const headingClass = "text-ink";
const textClass = "text-ink-2";
const mutedClass = "text-muted";
const tableHeadClass = "bg-surface-soft text-left text-xs uppercase text-muted";
const inputClass =
  "focus-ring mt-1 h-10 w-full rounded-md border border-line bg-surface px-3 py-0 leading-10 text-ink placeholder:text-muted";
const textareaClass =
...
const chartTooltipProps = {
  ...
  labelStyle: { color: "var(--stratis-teal)", fontWeight: 600 },
};
const axisTickStyle = {
  fill: "var(--chart-tick)",
  fontSize: 11,
...
const RISK_CLASS: Record<RiskLevel, string> = {
  ...
  Medium: "border-teal/25 bg-teal-soft text-teal",
  Low: "border-success/25 bg-success-soft text-success",
};
function formatChartCategoryLabel(value: string): string {
  const replacements: Record<string, string> = {
    "Bed Management": "Bed Mgmt",
    Documentation: "Docs",
    Equipment: "Equip",
    Medication: "Meds",
  };
...
function buildKpis(dataset: AppDataset): KpiMetric[] {
  const currentCases = dataset.cases.filter(
    (caseRecord) => (caseRecord.reportingPeriod ?? "Daily") === "Daily",
  );
  const planned = currentCases.length;
  const ready = currentCases.filter(
    (caseRecord) => caseRecord.dischargeReadiness === "Ready",
  ).length;
  const blocked = currentCases.filter(
    (caseRecord) => caseRecord.dischargeReadiness === "Blocked",
  ).length;
  const slaBreaches = currentCases.filter(hasSlaBreach).length;
  const activeCases = currentCases.filter(
    (caseRecord) => caseRecord.dischargeReadiness !== "Ready",
  );
  const avgBlockerAge =
    activeCases.length === 0
      ? 0
      : activeCases.reduce(
          (sum, caseRecord) => sum + caseRecord.blockerAgeHours,
          0,
        ) / activeCases.length;
  const avgBedRisk =
    currentCases.reduce(
      (sum, caseRecord) => sum + caseRecord.bedReleaseRisk,
      0,
    ) / Math.max(1, planned);
  const closedEscalations = currentCases.filter(
    (caseRecord) => caseRecord.escalationStatus === "Closed",
  ).length;
  const totalEscalations = currentCases.filter(
    (caseRecord) => caseRecord.escalationStatus !== "None",
  ).length;
  const closureRate =
    totalEscalations === 0
      ? 0
      : Math.round((closedEscalations / totalEscalations) * 100);
  return [
    {
      id: "planned",
      ...
      status: "Neutral",
      description:
        "Synthetic discharge cases loaded for today's operating cycle.",
    },
    {
      id: "ready",
    ...
    },
    {
      ...
      value: slaBreaches,
      status:
        slaBreaches > 2 ? "Critical" : slaBreaches > 0 ? "Warning" : "Good",
      description:
        "Operational blockers older than their target resolution SLA.",
    },
    {
      id: "age",
    ...
    },
    {
      ...
      status:
        avgBedRisk > 70 ? "Critical" : avgBedRisk > 45 ? "Warning" : "Good",
    ...
    },
    {
      ...
      value: `${closureRate}%`,
      status:
        closureRate >= 60 ? "Good" : closureRate >= 35 ? "Warning" : "Critical",
      description:
        "Closed escalations divided by open plus closed escalations.",
    },
  ];
}
function byCategory(recommendations: Recommendation[]) {
  const counts = recommendations.reduce<Record<BlockerCategory, number>>(
    (acc, recommendation) => {
...
function byDepartment(recommendations: Recommendation[]) {
  const counts = recommendations.reduce<Record<string, number>>(
    (acc, recommendation) => {
...
function ensureExtension(filename: string, extension: string): string {
  const cleaned =
    filename.trim() || `stratis-healthcare-ops-brief.${extension}`;
...
function collectBriefExportGraphs(): BriefExportGraph[] {
  ...
  return Array.from(
    document.querySelectorAll<HTMLElement>("[data-brief-export-chart]"),
  )
    .map((node, index) => {
...
function Shell({
  ...
  return (
    <div className="stratis-app-shell min-h-screen bg-paper text-ink">
      <header className="stratis-header border-b border-ink px-4 py-3 text-white shadow-sm">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 md:px-2">
          ...
          <nav className="flex gap-2 overflow-x-auto px-1 py-2.5 scroll-px-1">
            {" "}
            {PAGES.map((navPage) => (
              <button
                className={`focus-ring inline-flex items-center gap-2 whitespace-nowrap rounded-md border px-3 py-2 text-sm font-medium ${page === navPage ? "border-teal bg-teal text-white shadow-sm" : "border-white/15 bg-white/10 text-white/85 hover:border-white/30 hover:bg-white/15 hover:text-white"}`}
                key={navPage}
                onClick={() => setPage(navPage)}
                type="button"
              >
                <PageIcon page={navPage} /> {PAGE_LABELS[navPage]}{" "}
              </button>
            ))}{" "}
          </nav>
        </div>
...
function KpiCards({ metrics }: { metrics: KpiMetric[] }) {
  return (
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {metrics.map((metric) => (
        <article
          className={`rounded-md border p-4 shadow-sm ${STATUS_CLASS[metric.status]}`}
          key={metric.id}
        >
          ...
          <p className="mt-2 text-sm text-ink-2">{metric.description}</p>
        </article>
      ))}{" "}
    </section>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <div
          className={`${panelClass} flex min-h-[22rem] min-w-0 flex-col p-4`}
        >
            ...
            <div className="h-full min-h-[17rem] w-[620px] md:w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar
                    dataKey="Count"
                    fill="var(--stratis-chart-primary)"
                    radius={[4, 4, 0, 0]}
                  />
                </BarChart>
              </ResponsiveContainer>
      ...
      <section className={`${panelClass} p-4`}>
        ...
        <p className={`mt-1 text-sm ${mutedClass}`}>
          {" "}
          Count of current recommendations assigned to each accountable
          department.{" "}
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          {departmentData.map((department) => (
            <button
              className={`${panelSoftClass} focus-ring text-left transition hover:border-teal hover:shadow-panel`}
              key={department.name}
            ...
            >
              <div className="flex items-center justify-between p-3">
                ...
                <span className="rounded-full bg-teal-soft px-2 py-1 text-xs font-semibold text-teal">
                  {" "}
                  {department.Count}{" "}
                </span>
              </div>
        ...
        {selectedDepartment ? (
          ...
          >
            <div className="max-h-[85vh] w-full max-w-3xl overflow-auto rounded-md border border-line bg-surface p-4 shadow-modal">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className={`text-sm ${mutedClass}`}>
                    {" "}
                    Active recommendations owned by this department, excluding
                    resolved and rejected items.{" "}
                  </p>
                </div>
                <button
                  className="focus-ring rounded-md border border-line px-3 py-2 text-sm font-semibold text-ink hover:border-teal hover:text-teal"
              ...
              <div className="mt-4 grid gap-3">
                ...
                {selectedDepartmentItems.length > 0 ? (
                      ...
                      <div className="grid gap-3 p-3 md:grid-cols-[1fr_auto] md:items-start">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span
                              className={`rounded-full border px-2 py-1 text-xs font-semibold ${RISK_CLASS[recommendation.riskLevel]}`}
                            >
                              {" "}
                              {recommendation.riskLevel} |{" "}
                              {recommendation.priorityScore}{" "}
                            </span>
                            <span className="rounded-full border border-teal/25 bg-teal-soft px-2 py-1 text-xs font-semibold text-teal">
                              {" "}
                              {formatBlockerCategory(
                  ...
                  <p
                    className={`rounded-md bg-surface-soft p-3 text-sm ${mutedClass}`}
                  >
                    {" "}
...
function StatusMix({ dataset }: { dataset: AppDataset }) {
  const data = [
    ...
    },
    {
      name: "Pending",
      Count: dataset.cases.filter(
        (caseRecord) => caseRecord.dischargeReadiness === "Pending",
      ).length,
    },
    {
      name: "Blocked",
  ...
  const colors = [
    "var(--stratis-chart-primary)",
    "var(--stratis-chart-highlight)",
    "var(--stratis-danger)",
  ];
    ...
    <div className={`${panelClass} p-4`}>
      <div className="grid items-center gap-4 lg:grid-cols-[1fr_240px_190px]">
        <div>
          ...
          <p className={`text-sm ${mutedClass}`}>
            {" "}
            Operational split of discharge readiness. Use this with the table
            filters to isolate blocked cohorts.{" "}
          </p>
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            <div className="rounded-md bg-surface-soft p-3">
              ...
              <p className={`mt-1 text-xl font-semibold ${headingClass}`}>
                {Math.round((readyCount / Math.max(1, total)) * 100)}%
              </p>
            </div>
            <div className="rounded-md bg-surface-soft p-3">
              <p className="text-xs font-semibold uppercase text-muted">
        ...
        <div className="space-y-2">
          ...
          {data.map((entry, index) => (
            <div
              className="flex items-center justify-between rounded-md border border-line p-2"
              key={entry.name}
            >
...
function CommandCentre({ dataset }: { dataset: AppDataset }) {
  const [categoryFilter, setCategoryFilter] = useState<BlockerCategory | "All">(
    "All",
  );
  const [readinessFilter, setReadinessFilter] = useState<
    CaseRecord["dischargeReadiness"] | "All"
  >("All");
  ...
  const departments = Array.from(
    new Set(
      dataset.recommendations.map((recommendation) => recommendation.owner),
    ),
  ).sort();
  const rows = dataset.cases
    .map((caseRecord) => {
      const recommendation = dataset.recommendations.find(
        (rec) => rec.caseId === caseRecord.id,
      );
    ...
    .filter(({ caseRecord, recommendation }) => {
      ...
      const matchesReadiness =
        readinessFilter === "All" ||
        caseRecord.dischargeReadiness === readinessFilter;
      const matchesRisk =
        riskFilter === "All" || recommendation?.riskLevel === riskFilter;
      const matchesDepartment =
        departmentFilter === "All" ||
        recommendation?.owner === departmentFilter ||
        caseRecord.owner === departmentFilter;
      return (
        matchesCategory && matchesReadiness && matchesRisk && matchesDepartment
      );
    })
    .sort((a, b) => {
      if (!sortKey) return 0;
      ...
      const valueFor = (row: {
        ...
        if (sortKey === "category")
          return row.recommendation
            ? formatBlockerCategory(row.recommendation.category)
            : "N/A";
        if (sortKey === "owner")
          return row.recommendation?.owner ?? row.caseRecord.owner;
        if (sortKey === "sla")
          return (
            row.caseRecord.blockerAgeHours /
            Math.max(1, row.caseRecord.slaHours)
          );
  ...
  function SortIcon({ column }: { column: CommandSortKey }) {
    ...
    return (
      <svg
        aria-hidden="true"
        className={`h-3.5 w-3.5 ${active ? "text-teal" : "text-muted/70"}`}
        fill="none"
        viewBox="0 0 24 24"
      >
        <path
  ...
  return (
    <section className={panelClass}>
      <div className="border-b border-line p-4">
        ...
        <p className={`text-sm ${mutedClass}`}>
          Synthetic discharge workflow cases linked to recommendation status.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          <label className={`block text-sm font-medium ${textClass}`}>
            ...
            <select
              ...
              onChange={(event) =>
                setCategoryFilter(event.target.value as BlockerCategory | "All")
              }
            ...
            <tr>
              <th className="px-4 py-3">
                <button
                  ...
                  type="button"
                  onClick={() => updateSort("case")}
                >
                  CASE <SortIcon column="case" />
                </button>
              </th>
              <th className="px-4 py-3">
                <button
              ...
              <th className="px-4 py-3">
                <button
                  className={sortableHeaderClass}
                  type="button"
                  onClick={() => updateSort("readiness")}
                >
                  READINESS <SortIcon column="readiness" />
                </button>
              </th>
              <th className="px-4 py-3">
                <button
              ...
              <th className="px-4 py-3">
                ...
                >
                  DEPARTMENT <SortIcon column="owner" />
                </button>
              </th>
              <th className="px-4 py-3">
                <button
                  className={sortableHeaderClass}
                  type="button"
                  onClick={() => updateSort("sla")}
                >
                  SLA <SortIcon column="sla" />
                </button>
              </th>
              <th className="px-4 py-3">
                <button
                  className={sortableHeaderClass}
                  type="button"
                  onClick={() => updateSort("risk")}
                >
                  RISK | SCORE <SortIcon column="risk" />
                </button>
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-line">
            ...
            {rows.map(({ caseRecord, recommendation }) => (
              <tr key={caseRecord.id}>
                <td className={`px-4 py-3 font-medium ${headingClass}`}>
                  {caseRecord.id}
                </td>
                ...
                <td className={`max-w-md px-4 py-3 ${textClass}`}>
                  {caseRecord.blockerText}
                </td>
                <td className={`px-4 py-3 ${textClass}`}>
                  {recommendation?.owner ?? caseRecord.owner}
                </td>
                <td className={`px-4 py-3 ${textClass}`}>
                  {" "}
                ...
                <td className="px-4 py-3">
                  ...
                  {recommendation ? (
                    <span
                      className={`inline-flex max-w-[8rem] flex-wrap items-center gap-x-1 gap-y-0.5 rounded-md border px-2 py-1 text-xs font-semibold leading-tight sm:max-w-none sm:flex-nowrap ${RISK_CLASS[recommendation.riskLevel]}`}
                    >
...
function Queue({ recommendations }: { recommendations: Recommendation[] }) {
  ...
  const departments = Array.from(
    new Set(recommendations.map((recommendation) => recommendation.owner)),
  ).sort();
  const filteredRecommendations = recommendations
    .filter((recommendation) => {
      ...
      const matchesDepartment =
        departmentFilter === "All" || recommendation.owner === departmentFilter;
  ...
  const activeFilters = [
    categoryFilter !== "All"
      ? {
          label: `Category: ${formatBlockerCategory(categoryFilter)}`,
          clear: () => setCategoryFilter("All"),
        }
      : null,
    riskFilter !== "All"
      ? { label: `Risk: ${riskFilter}`, clear: () => setRiskFilter("All") }
      : null,
    statusFilter !== "All"
      ? {
          label: `Status: ${formatRecommendationStatus(statusFilter)}`,
  ...
  return (
    <section className="grid gap-4">
      <div className={panelClass}>
        <div className="flex flex-col justify-between gap-3 border-b border-line p-4 md:flex-row md:items-center">
          <div>
            ...
            <p className={`text-sm ${mutedClass}`}>
              {" "}
              Prioritised recommendations filtered by blocker category, risk,
              status, department, and case text.{" "}
            </p>
          </div>
          <button
            className="focus-ring rounded-md border border-line px-3 py-2 text-sm font-semibold text-ink hover:border-teal hover:text-teal"
        ...
        {activeFilters.length > 0 ? (
          <div className="flex flex-wrap gap-2 p-4">
            ...
            {activeFilters.map((filter) => (
              <button
                className="focus-ring inline-flex items-center gap-2 rounded-full border border-teal/25 bg-teal-soft py-1 pl-3 pr-1.5 text-xs font-semibold text-teal hover:border-teal/50"
                key={filter.label}
              ...
              >
                ...
                <span
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-teal/15 text-teal"
                  aria-hidden="true"
                >
                  <svg
                    viewBox="0 0 16 16"
          ...
          <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/50 px-4">
            <div className={`${panelClass} w-full max-w-2xl p-4`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  className={`focus-ring rounded-md px-2 py-1 text-sm font-semibold ${mutedClass}`}
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <label className={`block text-sm font-medium ${textClass}`}>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setCategoryFilter(
                        event.target.value as BlockerCategory | "All",
                      )
                    }
                    value={categoryFilter}
                  >
                    <option value="All">All Categories</option>{" "}
                    {Object.entries(BLOCKER_CATEGORY_LABELS).map(
                      ([value, label]) => (
                        <option key={value} value={value}>
                          {" "}
                          {label}{" "}
                ...
                <label className={`block text-sm font-medium ${textClass}`}>
                  ...
                  >
                    ...
                    {Object.entries(RECOMMENDATION_STATUS_LABELS).map(
                      ([value, label]) => (
                        <option key={value} value={value}>
                          {" "}
                          {label}{" "}
                ...
                <label
                  className={`block text-sm font-medium md:col-span-2 ${textClass}`}
                >
                  {" "}
                  ...
                  <input
                    ...
                    placeholder="Case ID, department, next step, or blocker category"
                    value={search}
                  />
                </label>
              </div>
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button
                  className="focus-ring rounded-md bg-teal px-3 py-2 text-sm font-semibold text-white hover:bg-teal-2"
                  onClick={() => setShowFilters(false)}
                  type="button"
                >
                  {" "}
      ...
      {filteredRecommendations.length > 0 ? (
        filteredRecommendations.map((recommendation) => (
          <article className={`${panelClass} p-4`} key={recommendation.id}>
            <div className="flex flex-col justify-between gap-3 md:flex-row">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {" "}
                    {recommendation.riskLevel} | Score{" "}
                    {recommendation.priorityScore}{" "}
                  </span>
                  <span className="rounded-full border border-line bg-surface-soft px-2 py-1 text-xs font-semibold text-ink-2">
                    {" "}
              ...
              <div className={`text-sm ${mutedClass} md:text-right`}>
                <p className={`font-medium ${headingClass}`}>
                  Department: {recommendation.owner}
                </p>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr]">
              ...
              <div className="rounded-md bg-teal-soft p-3">
        ...
        <div className={`${panelClass} p-4`}>
          <p className={`text-sm ${mutedClass}`}>
            No escalation recommendations match the selected filters.
          </p>
        </div>
      )}{" "}
    </section>
...
function HumanReview({
  ...
  const [assignedOwner, setAssignedOwner] = useState(
    dataset.recommendations[0]?.owner ?? DEPARTMENT_OPTIONS[0],
  );
  ...
  function submitDecision() {
    ...
    if (
      (action === "override" || action === "escalate") &&
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
      <div className={`${panelClass} p-4`}>
        ...
        <p className={`text-sm ${mutedClass}`}>
          Filter by Severity Level and Recommendation Status, then record the
        ...
        <div className="mt-4 space-y-4">
              ...
              {filteredRecommendations.length === 0 ? (
                ...
                filteredRecommendations.map((rec) => (
                  <option key={rec.id} value={rec.id}>
                    {" "}
                    {rec.caseId} - {rec.riskLevel} -{" "}
                    {formatRecommendationStatus(rec.status)} -{" "}
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className={`block text-sm font-medium ${textClass}`}>
              ...
              >
                {" "}
                {Object.entries(DECISION_ACTION_LABELS).map(
                  ([value, label]) => (
                    <option key={value} value={value}>
                      {" "}
                      {label}{" "}
          ...
          <button
            className="focus-ring rounded-md bg-teal px-4 py-2 text-sm font-semibold text-white hover:bg-teal-2"
        ...
        {recommendation ? (
          <div className={`mt-3 space-y-3 text-sm ${textClass}`}>
            <p>
              ...
              - {formatBlockerCategory(recommendation.category)} -{" "}
              {recommendation.riskLevel} Risk - Score{" "}
              {recommendation.priorityScore}{" "}
            </p>
            <p>{recommendation.recommendation}</p>
            <p className="rounded-md bg-teal-soft p-3 text-ink ">
              {" "}
              {recommendation.expectedNextStep}{" "}
            </p>
            <div className="rounded-md border border-line bg-surface-soft p-3 ">
              ...
              <p className={`mt-1 ${mutedClass}`}>
                {" "}
                Range: 0-100. Risk Bands: Low 0-44, Medium 45-69, High 70-84,
                Critical 85-100.{" "}
              </p>
              <p className={`mt-1 ${mutedClass}`}>
                {" "}
                Inputs: bed-release risk, SLA pressure, blocker age, open
                escalation state, and readiness state.{" "}
              </p>
            </div>
          ...
          <p className={`mt-3 text-sm ${mutedClass}`}>
            No Case Recommendation selected.
          </p>
        )}{" "}
      </div>
    </section>
...
function buildCycleGroups(dataset: AppDataset, timeframe: CycleTimeframe) {
  const multiplier: Record<CycleTimeframe, number> = {
    Daily: 1,
    Weekly: 5,
    Monthly: 21,
  };
  ...
  for (const recommendation of dataset.recommendations) {
    const caseRecord = dataset.cases.find(
      (item) => item.id === recommendation.caseId,
    );
    ...
    const contribution = Math.round(
      (caseRecord.blockerAgeHours + slaPenalty + bedRiskHours) *
        multiplier[timeframe],
    );
    const existing = groups.get(recommendation.category) ?? {
...
function KpiTree({ dataset }: { dataset: AppDataset }) {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<BlockerCategory | null>(null);
  const departmentData = byDepartment(dataset.recommendations);
  ...
  return (
    <section className="space-y-4">
      <div className={`${panelClass} p-4`}>
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
          <div>
            ...
            <p className={`text-sm ${mutedClass}`}>
              {" "}
              Cycle-time contribution links blocker age, SLA breach pressure,
              bed-release risk, category, and accountable department.{" "}
            </p>
          </div>
          <div className="flex rounded-md border border-line bg-surface-soft p-1 ">
            {" "}
            {(["Daily", "Weekly", "Monthly"] as CycleTimeframe[]).map(
              (option) => (
                <button
                  className={`focus-ring rounded px-3 py-1.5 text-sm font-semibold ${timeframe === option ? "bg-teal text-white" : "text-ink/80 hover:text-teal "}`}
                  key={option}
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_0.85fr]">
        <div className={`${panelClass} p-4`}>
          <h3
            className={`flex items-center gap-2 text-base font-semibold ${headingClass}`}
          >
            {" "}
            Cycle Time Contribution <ContributionInfoIcon />
          </h3>
          <p className={`text-sm ${mutedClass}`}>
            Click a category bar to inspect contributing cases.
          </p>
            ...
            <ResponsiveContainer height="100%" width="100%">
              ...
              >
                ...
                <YAxis
                  ...
                  tickMargin={4}
                  type="category"
                  width={cycleAxisWidth}
                />
                <Tooltip
                  {...chartTooltipProps}
                  formatter={(value) => [value, `${timeframe} Contribution`]}
                />
                <Bar
                  dataKey="Contribution"
                  fill="var(--stratis-chart-primary)"
                  radius={[0, 4, 4, 0]}
                  onClick={(group) => setSelectedCategory(group.category)}
                />
              </BarChart>
            </ResponsiveContainer>
          </div>
            ...
            {departmentData.map((department) => (
              <div className={panelSoftClass} key={department.name}>
                <div className="p-3">
                  ...
                  <p className={`text-sm ${mutedClass}`}>
                    {department.Count} Linked Blocker Recommendations
                  </p>
                </div>
              </div>
            ))}{" "}
      ...
      {selectedGroup ? (
        <div
          ...
          aria-modal="true"
        >
          <div className="max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-md border border-line bg-surface p-4 shadow-panel ">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className={`text-sm ${mutedClass}`}>
                  Contribution includes blocker age, SLA breach pressure, and
                  bed-release risk.
                </p>
              </div>
              <button
                className="focus-ring rounded-md border border-line px-3 py-2 text-sm font-semibold text-ink-2 "
            ...
            <div className="mt-4 space-y-3">
              ...
              {selectedGroup.items.map(
                ({ caseRecord, recommendation, contribution }) => (
                  <article className={panelSoftClass} key={caseRecord.id}>
                    <div className="p-3">
                      <div className="flex flex-col justify-between gap-2 md:flex-row">
                        <p className={`font-semibold ${headingClass}`}>
                          {" "}
                          {caseRecord.id} - {recommendation.riskLevel} Risk -
...
function formatEventDetailValue(
  value: GovernanceEvent["details"][string],
): string {
...
function confidenceExplanation(
  confidence: GovernanceEvent["details"][string],
): string {
  const value =
  ...
  return (
    ...
    >
      <button
        ...
        className="inline-flex h-[1em] w-[1em] items-center justify-center rounded-full border border-teal/30 text-teal align-middle "
      ...
      <span
        className={`absolute left-1/2 top-6 z-20 w-80 -translate-x-1/2 rounded-md border border-line bg-surface p-3 text-xs font-normal normal-case text-ink-2 shadow-panel transition-opacity ${visible ? "opacity-100" : "pointer-events-none opacity-0"}`}
      >
        {" "}
        Contribution points are synthetic operating-pressure units: blocker age
        hours, plus 3 points for an SLA breach, plus bed-release risk divided by
        20, multiplied by the selected Daily, Weekly, or Monthly timeframe.{" "}
      </span>
    </span>
  );
}
function ConfidenceInfoIcon({
  ...
  return (
    ...
    >
      ...
      <span
        className={`absolute left-1/2 top-6 z-20 w-72 -translate-x-1/2 rounded-md border border-line bg-surface p-3 text-xs normal-case text-ink-2 shadow-panel transition-opacity ${visible ? "opacity-100" : "pointer-events-none opacity-0"}`}
      >
        {" "}
...
function GovernanceDetailRows({ event }: { event: GovernanceEvent }) {
  ...
  if (event.details.categoryDisplay || event.details.category) {
    rows.push({
      label: "Category",
      value: event.details.categoryDisplay ?? event.details.category,
    });
  }
  ...
  if (event.details.owner || event.details.assignedOwner) {
    rows.push({
      label: "Department",
      value: event.details.assignedOwner ?? event.details.owner,
    });
  }
  if (event.details.confidence) {
    rows.push({
      label: "Confidence",
      value: `${Math.round(Number(event.details.confidence) * 100)}%`,
      showInfo: true,
    });
  }
  if (event.details.actionDisplay || event.details.action) {
    rows.push({
      ...
      value: event.details.actionDisplay ?? event.details.action,
    });
  }
      ...
      {rows.map((row) => (
        <div className="rounded-md bg-surface-soft p-3 " key={row.label}>
          <dt className="flex items-center gap-2 text-xs font-semibold uppercase text-muted ">
            ...
            {row.label}{" "}
            {row.showInfo ? (
              <ConfidenceInfoIcon confidence={event.details.confidence} />
            ) : null}{" "}
          </dt>
          <dd className={`mt-1 text-sm ${textClass}`}>
            {formatEventDetailValue(row.value)}
          </dd>
        </div>
      ))}{" "}
    </dl>
...
function GovernanceLog({
  ...
  const filteredEvents = events
    .filter((event) => {
      ...
      const matchesCategory =
        ...
        event.details.categoryDisplay === categoryLabel;
      const matchesRisk =
        riskLevel === "All" || event.details.riskLevel === riskLevel;
  ...
  const activeFilters = [
    ...
    riskLevel !== "All"
      ? {
          label: `Risk Level: ${RISK_LEVEL_LABELS[riskLevel]}`,
          clear: () => setRiskLevel("All"),
        }
      : null,
    dateFrom
      ? { label: `From: ${dateFrom}`, clear: () => setDateFrom("") }
      : null,
    ...
    <section className={panelClass}>
      <div className="flex flex-col justify-between gap-3 border-b border-line p-4 md:flex-row md:items-center ">
        <div>
          ...
          <p className={`text-sm ${mutedClass}`}>
            Recommendation and human decision events persisted in browser-local
            storage.
          </p>
        </div>
        <button
      ...
      {activeFilters.length > 0 ? (
        <div className="flex flex-wrap gap-2 border-b border-line p-4 ">
          ...
          <button
            className="focus-ring rounded-full border border-line px-3 py-1 text-xs font-semibold text-ink-2 "
            onClick={clearFilters}
            type="button"
          >
            {" "}
      ...
      <div className="divide-y divide-line ">
          ...
          <article className="p-4" key={event.id}>
            <div className="flex flex-col justify-between gap-2 md:flex-row">
              <div>
                ...
                <p className={`text-sm ${mutedClass}`}>
                  {" "}
                  {GOVERNANCE_EVENT_TYPE_LABELS[event.type]} | {event.actor} |{" "}
        ...
        {filteredEvents.length === 0 ? (
          <div className="p-4">
            <p className={`text-sm ${mutedClass}`}>
              No governance events match the selected filters.
            </p>
          </div>
        ) : null}{" "}
      </div>{" "}
      {showFilters ? (
        <div
          ...
          aria-modal="true"
        >
          <div className="w-full max-w-2xl rounded-md border border-line bg-surface p-4 shadow-panel ">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className={`text-lg font-semibold ${headingClass}`}>
                ...
                <p className={`text-sm ${mutedClass}`}>
                  Filter by creation date, event type, category, risk level,
                  case, or actor.
                </p>
              </div>
              <button
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <label className={`block text-sm font-medium ${textClass}`}>
                ...
                <select
                  className={inputClass}
                  onChange={(event) =>
                    setEventType(event.target.value as GovernanceEventFilter)
                  }
                  value={eventType}
                >
              ...
              <label className={`block text-sm font-medium ${textClass}`}>
                ...
                <select
                  className={inputClass}
                  onChange={(event) =>
                    setCategory(event.target.value as GovernanceCategoryFilter)
                  }
                  value={category}
                >
                  <option value="All">All Categories</option>{" "}
              ...
              <label className={`block text-sm font-medium ${textClass}`}>
                ...
                <select
                  className={inputClass}
                  onChange={(event) =>
                    setRiskLevel(event.target.value as GovernanceRiskFilter)
                  }
                  value={riskLevel}
                >
                  <option value="All">All Risk Levels</option>{" "}
              ...
              <label className={`block text-sm font-medium ${textClass}`}>
                ...
                <input
                  className={inputClass}
                  onChange={(event) => setSearch(event.target.value)}
                  placeholder="Case ID, actor, recommendation ID"
                  value={search}
                />
              </label>
            </div>
...
function parseBriefKeyValueRows(
  lines: string[],
): Array<{ name: string; value: number; rawValue: string }> {
  return lines
    .map((line) => {
...
function BriefExportCharts({ brief }: { brief: string }) {
  ...
  const statusRows = parseBriefKeyValueRows(
    parsed.sections["Current Operating Status"] ?? [],
  );
  const blockerRows = parseBriefKeyValueRows(
    parsed.sections["Top Blockers"] ?? [],
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      ...
      {statusChartData.length > 0 ? (
        <div
          ...
          data-brief-export-chart
          data-chart-title="Current Operating Status"
        >
          <h3 className={`text-base font-semibold ${headingClass}`}>
            Current Operating Status Chart
          </h3>
...
function BriefBulletCard({ line }: { line: string }) {
  ...
  return (
    <article className="rounded-md border border-line bg-surface-soft p-3 ">
      <p className={`text-sm font-semibold ${headingClass}`}>{title}</p>{" "}
      {detail ? (
        <p className={`mt-1 text-sm leading-6 ${textClass}`}>{detail}</p>
      ) : null}{" "}
    </article>
  );
}
function FormattedBrief({ brief }: { brief: string }) {
  ...
  const maxBlockerCount = Math.max(
    1,
    ...topBlockers
      .map((line) => Number(line.split(":").pop()?.trim() ?? 0))
      .filter((value) => !Number.isNaN(value)),
  );
  const standardSections = Object.entries(parsed.sections).filter(
    ([section]) =>
      section !== "Current Operating Status" && section !== "Top Blockers",
  );
  return (
    <div className="space-y-4 p-4">
        ...
        <section className="rounded-md border border-line p-4 ">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            {currentStatus.map((line) => {
              ...
              return (
                <div className="rounded-md bg-surface-soft p-3 " key={line}>
                  <p className="text-xs font-semibold uppercase text-muted ">
                    {label}
                  </p>
                  <p className={`mt-1 text-xl font-semibold ${headingClass}`}>
                ...
                <div key={line}>
                  ...
                  <div className="h-2 rounded-full bg-surface-soft ">
                    <div
                      ...
                      style={{
                        width: `${Math.max(8, (value / maxBlockerCount) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
      ...
      {standardSections.map(([section, lines]) => (
        <section className="rounded-md border border-line p-4 " key={section}>
          <h3 className={`text-base font-semibold ${headingClass}`}>
            {section}
          </h3>
  ...
  return (
    <section className={panelClass}>
      <div className="border-b border-line p-4 ">
        <div>
          <h2 className={`text-lg font-semibold ${headingClass}`}>
            {duration} {detailMode} Operating Brief
          </h2>
          <p className={`text-sm ${mutedClass}`}>
            Generated from synthetic operating data, decisions, and governance
            events.
          </p>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <label className={`block text-sm font-medium ${textClass}`}>
            ...
            <select
              className={inputClass}
              onChange={(event) =>
                onDurationChange(event.target.value as BriefDuration)
              }
              value={duration}
            >
          ...
          <label className={`block text-sm font-medium ${textClass}`}>
            ...
            <select
              className={inputClass}
              onChange={(event) =>
                onDetailModeChange(event.target.value as BriefDetailMode)
              }
              value={detailMode}
            >
          ...
          <div>
            ...
            <div className="mt-1 flex h-10 rounded-md border border-line bg-paper p-1 ">
              {" "}
              {(["Text", "Markdown"] as BriefViewMode[]).map((mode) => (
                <button
                  className={`focus-ring flex-1 rounded px-3 text-sm font-semibold ${viewMode === mode ? "bg-teal text-white" : "text-ink/80 hover:text-teal "}`}
                  key={mode}
      ...
      {viewMode === "Markdown" ? (
        <pre className="whitespace-pre-wrap p-4 text-sm leading-6 text-ink-2 ">
          {brief}
        </pre>
      ) : (
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none fixed -left-[10000px] top-0 w-[900px] bg-white text-[#14212b]"
      >
        <div ref={exportRef}>
          <FormattedBrief brief={brief} />
        </div>
      ...
      {showExportModal ? (
        <div
          ...
          aria-modal="true"
        >
          <div className="w-full max-w-lg rounded-md border border-line bg-surface p-4 shadow-panel ">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className={`text-lg font-semibold ${headingClass}`}>
                  Export Operating Brief
                </h3>
                <p className={`mt-1 text-sm ${mutedClass}`}>
                  Choose a format and filename. PDF and DOCX exports are
                  generated in the browser and include the brief charts.
                </p>
              </div>
              <button
                className="focus-ring rounded-md px-2 py-1 text-sm font-semibold text-muted hover:text-ink "
                ...
                <select
                  ...
                  onChange={(event) =>
                    handleExportFormatChange(
                      event.target.value as BriefExportFormat,
                    )
                  }
                  value={exportFormat}
                >
...
function Settings({
  ...
  const [activeSetting, setActiveSetting] = useState<
    "Appearance" | "Data Controls" | "Governance" | "Taxonomy" | "Guardrails"
  >("Appearance");
  const [newCategoryLabel, setNewCategoryLabel] = useState("");
  const [newEventTypeLabel, setNewEventTypeLabel] = useState("");
  const [taxonomyError, setTaxonomyError] = useState("");
  const [pendingDelete, setPendingDelete] =
    useState<PendingTaxonomyDelete | null>(null);
  ...
  function addOption(
    ...
    if (
      options.some(
        (option) =>
          normalizeConfigLabel(option.label) === normalized ||
          normalizeConfigLabel(option.value) === normalized,
      )
    ) {
      setTaxonomyError(
        "That item already exists. Duplicate checks ignore case and spaces.",
      );
      return;
    }
    update(
      sortConfigOptions([
        ...options,
        { value: `custom_${normalized}`, label: trimmed },
      ]),
    );
  ...
  function removeOption(
    ...
    update(
      sortConfigOptions(options.filter((option) => option.value !== value)),
    );
    if (removed) {
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[240px_1fr]">
      <aside className={`${panelClass} p-3`}>
        <p className="px-2 pb-2 text-xs font-semibold uppercase text-muted ">
          Settings
        </p>
        <nav className="space-y-1">
          ...
          {settingsSections.map((section) => (
            <button
              className={`focus-ring w-full rounded-md px-3 py-2 text-left text-sm font-semibold ${activeSetting === section ? "bg-teal text-white" : "text-ink/80 hover:bg-paper hover:text-teal "}`}
              key={section}
      ...
      <div className={`${panelClass} p-4`}>
        ...
        {activeSetting === "Appearance" ? (
          <div>
            ...
            <p className={`mt-2 text-sm ${mutedClass}`}>
              {" "}
              STRATIS visual mode controls the local workbench only and is saved
              in this browser.{" "}
            </p>
            <div className="mt-4 flex rounded-md border border-line bg-paper p-1 ">
              {" "}
              {(["light", "dark"] as ThemeMode[]).map((mode) => (
                <button
                  className={`focus-ring flex-1 rounded px-3 py-2 text-sm font-semibold ${theme === mode ? "bg-teal text-white" : "text-ink/80 hover:text-teal "}`}
                  key={mode}
        ...
        {activeSetting === "Data Controls" ? (
          <div>
            ...
            <p className={`mt-2 text-sm ${mutedClass}`}>
              {" "}
              The MVP uses synthetic discharge operations data only. Reset
              returns the browser workspace to the built-in sample dataset.{" "}
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                className="focus-ring rounded-md border border-line px-3 py-2 text-sm font-semibold text-ink hover:border-teal hover:text-teal "
              ...
              <button
                className="focus-ring rounded-md bg-ink px-3 py-2 text-sm font-semibold text-white hover:bg-ink-2 "
                onClick={onReset}
                type="button"
              >
                {" "}
        ...
        {activeSetting === "Governance" ? (
          <div>
            ...
            <p className={`mt-2 text-sm ${mutedClass}`}>
              {" "}
              Governance events are stored locally and displayed in
              layperson-readable cards. Backend-safe event details remain
              structured for export and audit.{" "}
            </p>
            <ul className={`mt-4 space-y-2 text-sm ${textClass}`}>
              <li>
                - Recommendation creation, human decisions, dataset resets, and
                brief generation are logged.
              </li>
              <li>
                - Override and Escalation decisions require reviewer rationale.
              </li>
              <li>
                - Production hardening would require immutable server-side audit
                storage.
              </li>
            </ul>
          </div>
        ...
        {activeSetting === "Taxonomy" ? (
          <div>
            ...
            <p className={`mt-2 text-sm ${mutedClass}`}>
              {" "}
              Configure frontend filter options for Governance Log event types
              and blocker categories. These labels do not rewrite historical
              audit records.{" "}
            </p>{" "}
            {taxonomyError ? (
              <p className="mt-3 rounded-md bg-danger-soft px-3 py-2 text-sm font-semibold text-danger ">
                {" "}
                {taxonomyError}{" "}
              </p>
            ) : null}{" "}
              ...
              <section className="rounded-md border border-line p-3 ">
                ...
                <div className="mt-3 flex gap-2">
                  <input
                    ...
                    onChange={(event) =>
                      setNewEventTypeLabel(event.target.value)
                    }
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {eventTypeOptions.map((option) => (
                    <div
                      className="flex items-center justify-between rounded-md bg-surface-soft p-2 "
                      key={option.value}
                    >
                      ...
                      <button
                        className="focus-ring rounded px-2 py-1 text-xs font-semibold text-danger "
                        onClick={() =>
                          requestRemoveOption(option, "Event Type")
                        }
                        type="button"
                      >
                        {" "}
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {categoryOptions.map((option) => (
                    ...
                    >
                      ...
                      <button
                        className="focus-ring rounded px-2 py-1 text-xs font-semibold text-danger "
                        onClick={() => requestRemoveOption(option, "Category")}
                        type="button"
                      >
                        {" "}
                        Delete{" "}
            ...
            <section className="mt-4 rounded-md border border-line p-3 ">
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  className={`text-xs font-semibold uppercase ${mutedClass}`}
                >
                  Browser Local
                </span>
              </div>
              <div className="mt-3 space-y-2">
                ...
                {taxonomyChanges.length > 0 ? (
                  taxonomyChanges.slice(0, 8).map((change) => (
                    <div
                      className="grid gap-2 rounded-md bg-surface-soft p-2 text-sm md:grid-cols-[150px_90px_110px_1fr]"
                      key={change.id}
                    >
                      ...
                      <span
                        className={`font-semibold ${change.action === "Added" ? "text-success " : "text-danger "}`}
                      >
                        {" "}
                        {change.action}{" "}
                      </span>
        ...
        {activeSetting === "Guardrails" ? (
          <div>
            ...
            <ul
              className={`mt-3 grid gap-2 text-sm ${textClass} md:grid-cols-2`}
            >
              <li>- Synthetic operations data only.</li>
              <li>
                - No clinical diagnosis, treatment guidance, or patient advice.
              </li>
              <li>
                - Deterministic rules engine; AI integration is future
                optionality.
              </li>
              <li>- Browser-local persistence; no backend or secrets.</li>
              <li>
                - Governance log captures recommendations and human decisions.
              </li>
            </ul>
          </div>
        ) : null}{" "}
      ...
      {pendingDelete ? (
        ...
        >
          <div className="w-full max-w-md rounded-md border border-line bg-surface p-4 shadow-modal ">
            <h3
              className={`text-lg font-semibold ${headingClass}`}
              id="taxonomy-delete-title"
            >
              {" "}
            ...
            <p className={`mt-2 text-sm ${textClass}`}>
              {" "}
              You are about to delete the {pendingDelete.taxonomy.toLowerCase()}{" "}
              item{" "}
              <span className={`font-semibold ${headingClass}`}>
                {pendingDelete.label}
              </span>
            ...
            <p className={`mt-2 text-sm ${mutedClass}`}>
              {" "}
              This removes it from future filter configuration only. Historical
              governance records are not rewritten.{" "}
            </p>
            <div className="mt-4 flex justify-end gap-2">
              <button
                className="focus-ring rounded-md border border-line px-3 py-2 text-sm font-semibold text-ink hover:border-teal hover:text-teal "
              ...
              <button
                className="focus-ring rounded-md bg-danger px-3 py-2 text-sm font-semibold text-white hover:bg-danger"
...
function App() {
  ...
  const [briefDetailMode, setBriefDetailMode] =
    useState<BriefDetailMode>("Executive");
  ...
  const [categoryOptions, setCategoryOptions] = useState<ConfigOption[]>(() => {
    ...
    const raw = window.localStorage.getItem(
      "stratis-healthcare-category-options",
    );
    return raw
      ? sortConfigOptions(JSON.parse(raw) as ConfigOption[])
  ...
  const [eventTypeOptions, setEventTypeOptions] = useState<ConfigOption[]>(
    () => {
      ...
      const raw = window.localStorage.getItem(
        "stratis-healthcare-event-type-options",
      );
      return raw
        ? sortConfigOptions(JSON.parse(raw) as ConfigOption[])
  ...
  const [taxonomyChanges, setTaxonomyChanges] = useState<TaxonomyChange[]>(
    () => {
      ...
      const raw = window.localStorage.getItem(
        "stratis-healthcare-taxonomy-changes",
      );
  ...
  const [theme, setTheme] = useState<ThemeMode>(() => {
    ...
    return window.localStorage.getItem("stratis-healthcare-theme") === "dark"
      ? "dark"
      : "light";
  });
  useEffect(() => {
  ...
  useEffect(() => {
    window.localStorage.setItem(
      "stratis-healthcare-event-type-options",
      JSON.stringify(eventTypeOptions),
    );
  }, [eventTypeOptions]);
  useEffect(() => {
    window.localStorage.setItem(
      "stratis-healthcare-taxonomy-changes",
      JSON.stringify(taxonomyChanges),
    );
  }, [taxonomyChanges]);
  useEffect(() => {
  ...
  function updateCaseForDecision(
    cases: CaseRecord[],
  ...
  function handleDecision(
    ...
    const nextDataset: AppDataset = {
      ...
      recommendations: setRecommendationStatus(
        dataset.recommendations,
        recommendation.id,
        action,
        assignedOwner,
      ...
      governanceEvents: [
        { ...event, details: { ...event.details, assignedOwner } },
  ...
  function handleGenerateBrief() {
    const nextBrief = generateBrief(dataset, {
      duration: briefDuration,
      detailMode: briefDetailMode,
    });
    setBrief(nextBrief);
    persistDataset({
      ...
      governanceEvents: [
        createSystemEvent(
          "brief_generated",
          `${briefDuration} ${briefDetailMode} operating brief generated.`,
          {
  ...
  async function handleExportBrief(
    ...
    if (format === "Markdown") {
      ...
      await saveBlob(
        filename,
        new Blob([briefToMarkdownWithGraphs(brief, graphs)], {
          type: "text/markdown",
        }),
        "Markdown",
        [".md"],
    ...
    try {
      await saveBlob(filename, await createBriefPdf(exportElement), "PDF", [
        ".pdf",
      ]);
    } finally {
  ...
  function renderPage() {
    ...
    return (
      <Settings
        ...
        onExportCases={() =>
          downloadText(
            "stratis-healthcare-ops-synthetic-cases.csv",
            casesToCsv(dataset.cases),
            "text/csv",
          )
        }
